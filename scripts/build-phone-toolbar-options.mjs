#!/usr/bin/env node
/**
 * Render docs/design/phone-toolbar-options.html — the decision page for where
 * the page tools live on a phone (docs/decisions/phone-toolbar.md).
 *
 * The three layouts differ in something a picture cannot carry: how far the
 * thumb travels, whether the page's last lines stay in view, and whether the
 * tools are still there on the next tap. So the page draws nothing. It mounts
 * the real app three times, phone-sized, one layout each — the app picks its
 * layout from `?phonebar=a|b|c` in its own address — and the reader decides by
 * holding each.
 *
 ...
 * frame says so and names the command that serves one.
 *
 * ── No Qur'an ───────────────────────────────────────────────────────────────
 * Everything a reader reads here is English chrome. The writer refuses if its
 * own output carries an Arabic codepoint.
 *
 * Registered in docs/decisions.json as the `builtBy` for phone-toolbar.
 *
 *   node scripts/build-phone-toolbar-options.mjs
 */
import { writeFileSync } from "node:fs";
import { join } from "node:path";
import { ROOT } from "./code-pointers.mjs";

const OUT = join(ROOT, "docs/design/phone-toolbar-options.html");

const OPTIONS = [
  {
    id: "a",
    name: "A · A strip under the top bar",
    gist: "Always there. One tap to any tool.",
    pros: ["One tap switches tools, and every tool is always in sight.", "Nothing to open or close, so nothing to learn."],
    cons: [
      "It takes a row of the screen for good: on a phone the page's last lines drop below the fold.",
      "It sits at the top, the hardest place for a thumb to reach one-handed.",
    ],
    then: "The page shrinks by a row on every phone, whether or not the reader ever uses a tool.",
  },
  {
    id: "b",
    name: "B · A pen case in the bottom row",
    gist: "One button that shows the tool that is on; a tap fans the tools out, with their names.",
    pros: [
      "Costs no room: the page is as big as it is today.",
      "The button's picture says which tool is on, and the fan names every tool.",
    ],
    cons: [
      "Two taps to switch tools, every time.",
      "The fan covers part of the page while it is open.",
    ],
    then: "The one button has to carry the whole state of the tools; a reader who forgets a tool is on sees only its small picture.",
  },
  {
    id: "c",
    name: "C · A tray that slides up from the bottom row",
    gist: "A Tools button by the thumb; the tray stays up, with a line saying what the tool on does, until it is closed.",
    pros: [
      "Costs no room while closed, and sits where the thumb already is.",
      "While open, switching is one tap, and the line under the tools says what a tap on the page will do.",
      "Closing it puts the page back to plain reading, so a tool is never left on by accident.",
    ],
    cons: [
      "While open it covers the bottom row: the page bar and the bookmarks are out of reach until it closes.",
      "Two taps to the first tool.",
    ],
    then: "Tool use becomes a mode the reader steps into and out of; the bottom row is where that mode lives.",
  },
];
...
<title>Page tools on a phone</title>
<style>
  :root{
...
<body>
<main>
<h1>Where should the page tools sit on a phone?</h1>
...
<dl class="glossary">
  ...
  <dt>Vowel-sign</dt><dd>the small mark above or below a letter that says how to sound it.</dd>
  <dt>The bottom row</dt><dd>the strip under the page that holds the page bar and the bookmarks.</dd>
  <dt>Below the fold</dt><dd>off the bottom of the screen, so the reader has to scroll to see it.</dd>
</dl>

<h2>What should you try in each?</h2>
<p>Pick the Word tool, then tap a word on the page: it opens into its parts. Pick Mistake and tap a word: it is marked.
Then go back to plain reading. Notice how far your thumb went, whether the page's last line stayed in view, and
whether you could tell which tool was on.</p>
...
<script>
  ...
  (function () {
    ...
    document.querySelectorAll("iframe[data-bar]").forEach(function (f) {
      var id = f.getAttribute("data-bar");
      var src = app.href + "?phonebar=" + id + "#/hafs-kfqc/p7";
      f.src = src;
      f.hidden = false;
...
if (/[\u0600-\u06FF\u0750-\u077F\u08A0-\u08FF\uFB50-\uFDFF\uFE70-\uFEFF]/.test(html))
  throw new Error("build-phone-toolbar-options: the page carries an Arabic codepoint");
