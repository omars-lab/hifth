 ...
 * exactly where the search put it, `box + (dx, dy)`. If it did not, the mark
 * inherits the printed line's own drift — the `line-tilt` correction fitted on
 * that page — which is the fallback that makes H shippable rather than reckless:
 * a mark chasing weak ink is set down on the line instead of on the noise.
 *
 ...
 * `ink` (its own found ink), `tilt` (the line's fallback), or `hand` (a person).
 * Marks appear in reading order within each ayah.
 *
...
for (const page of pages) {
  ...
  for (const m of ms) {
    ...
    const d = refused ? corr.apply(row) : { dx: row.dx, dy: row.dy };
    let rect = [r1(m.box[0] + d.dx), r1(m.box[1] + d.dy), r1(m.box[2]), r1(m.box[3])];
    let src = refused ? "tilt" : "ink";
    ...
    tally.marks += 1;
    tally[src === "hand" ? "hand" : refused ? "tilt" : "ink"] += 1;
  }
...
if (!only) {
  const pin = {
    ...
    $rule:
      ...
      "when its match clears IoU " + RULE.iou + " and the search kept room (its own searchedAt); otherwise " +
      "it inherits the printed line's tilt. Hand placements from the rulings win over both.",
    rule: { iou: RULE.iou, radius: RULE.radius, fallback: "line-tilt" },
    rows: {
...
console.log(
  `\nbuild:mark-placements — ${pinPages.length} shard(s), ${totals.marks} marks\n` +
    `  ${totals.ink} on their own ink · ${totals.tilt} on the line's tilt · ${totals.hand} by hand (of ${placements.size} placements)\n` +
