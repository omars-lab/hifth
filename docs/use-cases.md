...
flowchart LR
  subgraph ac_hafiz_g["الحافظ · the hafiz"]
    ...
    uc_see_what_i_have_not_opened_lately("Keep a private record of what I<br/>have actually opened, so I can<br/>tell which pages and juz have gone<br/>weeks without me.")
...
| happens in | [`apps/web/src/components/PageStage.tsx:336`](../apps/web/src/components/PageStage.tsx#L336) — `async navigateTo` |
...
### Keep a private record of what I have actually opened, so I can tell which pages and juz have gone weeks without me.

`see-what-i-have-not-opened-lately` · change it with `make map FEATURE=revision-record` · needs first: `select-the-ayah-i-am-on`

| | |
|---|---|
| happens in | [`packages/core/src/revision.ts:168`](../packages/core/src/revision.ts#L168) — `lastSeen` |
| happens in | [`packages/core/src/revision.ts:111`](../packages/core/src/revision.ts#L111) — `scopesOf` |
...
| proven by | [`packages/core/src/revision.test.ts:137`](../packages/core/src/revision.test.ts#L137) — “answers with the most recent day, whatever order the log is in” |
...
> **Not proven:** Two holes, and the first is not closable in this repo. iOS deletes script-writable storage after seven days of no interaction, which is precisely the history this promise is about — every test here runs against fake-indexeddb in Node, where nothing evicts, so what is proven is that the record *knows how young it is* (`since`), not that it survives. Whether an installed PWA on a real iPhone keeps it is docs/validation/ledger.json → offline-survival-8-day. Second, the marquee path is proven at the store, not through the gesture: no unit test drives a real drag, so «one look, not twelve» is asserted on `recordLook` rather than on a release over twelve ayahs. And there is no picture yet — task #91 is gated on Loop 4b, because a page heatmap over 3 vendored pages of 604 draws a mostly-cold book.
...
| happens in | [`apps/web/src/format.ts:105`](../apps/web/src/format.ts#L105) — `export function digits` |
...
| happens in | [`apps/web/src/i18n.tsx:785`](../apps/web/src/i18n.tsx#L785) — `LangProvider` |
| happens in | [`apps/web/src/components/Colophon.tsx:193`](../apps/web/src/components/Colophon.tsx#L193) — `langRow` |
...
| | |
|---|---|
| happens in | [`apps/web/src/components/Colophon.tsx:49`](../apps/web/src/components/Colophon.tsx#L49) — `CREDITS` |
...
- **Keep a private record of what I have actually opened, so I can tell which pages and juz have gone weeks without me.** — Two holes, and the first is not closable in this repo. iOS deletes script-writable storage after seven days of no interaction, which is precisely the history this promise is about — every test here runs against fake-indexeddb in Node, where nothing evicts, so what is proven is that the record *knows how young it is* (`since`), not that it survives. Whether an installed PWA on a real iPhone keeps it is docs/validation/ledger.json → offline-survival-8-day. Second, the marquee path is proven at the store, not through the gesture: no unit test drives a real drag, so «one look, not twelve» is asserted on `recordLook` rather than on a release over twelve ayahs. And there is no picture yet — task #91 is gated on Loop 4b, because a page heatmap over 3 vendored pages of 604 draws a mostly-cold book.
