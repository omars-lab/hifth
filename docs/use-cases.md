...
flowchart LR
  ...
  class uc_select_the_ayah_i_am_on,uc_select_a_whole_passage,uc_hop_to_something_that_resembles_it,uc_hop_from_a_passage,uc_see_exactly_what_differs,uc_find_ayahs_sharing_a_root,uc_get_back_to_where_i_was,uc_send_someone_what_i_am_looking_at,uc_open_a_link_someone_sent_me,uc_jump_to_a_place_i_can_name,uc_turn_the_page,uc_learn_the_gestures_once,uc_read_with_tajweed_colouring,uc_read_the_print_i_memorised_from,uc_revise_with_no_network,uc_use_it_without_seeing_it,uc_know_where_this_text_came_from,uc_run_a_check_a_machine_cannot,uc_bank_a_verdict_so_it_cannot_be_lost,uc_find_where_a_feature_lives,uc_know_what_we_promise_and_what_proves_it,uc_rebuild_the_hop_edges_from_the_dataset,uc_vendor_another_mushaf_page,uc_refuse_a_change_that_alters_the_picture,uc_refuse_a_change_that_loses_the_licence_trail,uc_refuse_a_doc_that_points_at_nothing,uc_refuse_a_bundle_that_would_not_load_on_a_phone uc;
```
...
| proven by | [`apps/web/e2e/marquee.spec.ts:117`](../apps/web/e2e/marquee.spec.ts#L117) — “tap-to-select still works alongside the marquee” |
...
| happens in | [`apps/web/src/components/PageStage.tsx:100`](../apps/web/src/components/PageStage.tsx#L100) — `PageStage` |
...
| proven by | [`apps/web/e2e/marquee.spec.ts:96`](../apps/web/e2e/marquee.spec.ts#L96) — “an immediate drag still pans the page and highlights nothing” |
...
| happens in | [`apps/web/src/components/PageStage.tsx:335`](../apps/web/src/components/PageStage.tsx#L335) — `async navigateTo` |
...
| happens in | [`apps/web/src/components/DiffView.tsx:38`](../apps/web/src/components/DiffView.tsx#L38) — `DiffView` |
...
| happens in | [`apps/web/src/components/RootLens.tsx:130`](../apps/web/src/components/RootLens.tsx#L130) — `RootLens` |
...
|---|---|
| happens in | [`packages/core/src/router.ts:95`](../packages/core/src/router.ts#L95) — `serializeState` |
| happens in | [`apps/web/src/components/ShareSheet.tsx:26`](../apps/web/src/components/ShareSheet.tsx#L26) — `ShareSheet` |
...
| | |
|---|---|
| happens in | [`packages/core/src/jump.ts:134`](../packages/core/src/jump.ts#L134) — `parseJump` |
| happens in | [`apps/web/src/components/Jumper.tsx:64`](../apps/web/src/components/Jumper.tsx#L64) — `Jumper` |
...
| | |
|---|---|
| happens in | [`apps/web/src/components/CoachMarks.tsx:69`](../apps/web/src/components/CoachMarks.tsx#L69) — `CoachMarks` |
...
| | |
|---|---|
| happens in | [`packages/core/src/concordance.ts:51`](../packages/core/src/concordance.ts#L51) — `EDITIONS` |
| happens in | [`packages/core/src/concordance.ts:114`](../packages/core/src/concordance.ts#L114) — `class Concordance` |
...
| happens in | [`apps/web/src/format.ts:42`](../apps/web/src/format.ts#L42) — `toArabicDigits` |
...
| proven by | [`apps/web/e2e/share-a11y.spec.ts:201`](../apps/web/e2e/share-a11y.spec.ts#L201) — “the mushaf page is a labelled group of ayah buttons” |
| proven by | [`apps/web/e2e/share-a11y.spec.ts:191`](../apps/web/e2e/share-a11y.spec.ts#L191) — “the chrome announces every control by word, never by glyph” |
...
| | |
|---|---|
| happens in | [`apps/web/src/components/Colophon.tsx:48`](../apps/web/src/components/Colophon.tsx#L48) — `CREDITS` |
...
| happens in | [`scripts/build-validation-guide.mjs:349`](../scripts/build-validation-guide.mjs#L349) — `GUIDE_PATH` |
...
| happens in | [`scripts/gate-validation.mjs:80`](../scripts/gate-validation.mjs#L80) — `needsRunbook` |
| proven by | `pnpm gate:validation` |
...
| | |
|---|---|
| happens in | [`scripts/gate-use-cases.mjs:139`](../scripts/gate-use-cases.mjs#L139) — `problems` |
| happens in | [`scripts/build-use-cases.mjs:62`](../scripts/build-use-cases.mjs#L62) — `mermaid` |
...
| happens in | [`packages/core/src/quran-meta.ts:45`](../packages/core/src/quran-meta.ts#L45) — `toAbsoluteAyah` |
| proven by | `pnpm gate:edges` |
| proven by | `pnpm gate:ci-artifacts` |

> **Not proven:** The gate proves an edge shares words with its source — it cannot prove the pair is a mutashabih a hafiz would recognise. That is ledger check edge-spot-audit, against a printed mushaf.
...
| | |
|---|---|
| happens in | [`apps/web/playwright.config.ts:73`](../apps/web/playwright.config.ts#L73) — `snapshotPathTemplate` |
...
| happens in | [`scripts/gate-license-copy.mjs:42`](../scripts/gate-license-copy.mjs#L42) — `NON_COMMERCIAL` |
...
| happens in | [`.github/workflows/ci.yml:39`](../.github/workflows/ci.yml#L39) — `build-test-gate` |
| proven by | `pnpm gate:budget` |

## What is promised more than it is proven

6 of 27 use cases carry a `gap` — a promise whose proof is thinner than the promise. They are collected here because a hole listed once beside its own use case is a hole nobody counts.
...
- **Regenerate the mutashabihat shards and know the output is the same as anyone else would have produced.** — The gate proves an edge shares words with its source — it cannot prove the pair is a mutashabih a hafiz would recognise. That is ledger check edge-spot-audit, against a printed mushaf.
