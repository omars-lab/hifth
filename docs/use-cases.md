...
| happens in | [`apps/web/src/components/PageStage.tsx:326`](../apps/web/src/components/PageStage.tsx#L326) — `PageStage` |
...
| happens in | [`apps/web/src/components/PageStage.tsx:1521`](../apps/web/src/components/PageStage.tsx#L1521) — `applyWords` |
| proven by | [`apps/web/e2e/word.spec.ts:90`](../apps/web/e2e/word.spec.ts#L90) — “a hold inside the selected ayah drops to words; a drag extends the run” |
| proven by | [`apps/web/e2e/word.spec.ts:132`](../apps/web/e2e/word.spec.ts#L132) — “Escape climbs back to the whole ayah, not out of the selection” |
| proven by | [`apps/web/e2e/word.spec.ts:149`](../apps/web/e2e/word.spec.ts#L149) — “the same hold outside the selection still paints a marquee” |
| proven by | `pnpm gate:words` |

> **Not proven:** The words can be picked and nothing is done with them yet: the hop still searches at ayah granularity, because print words and the QAC segmentation the edges are built from do not line up one-to-one (docs/PLAN.md 14). And the whole descent is finger-only — there is no keyboard or screen-reader path down to a word, so a reader who cannot see the page cannot reach this at all. Tracked, not assumed covered: docs/issues.json word-selection-has-no-keyboard-or-screen-reader-path.
...
| happens in | [`apps/web/src/components/PageStage.tsx:1262`](../apps/web/src/components/PageStage.tsx#L1262) — `async navigateTo` |
...
| proven by | [`apps/web/e2e/share-a11y.spec.ts:99`](../apps/web/e2e/share-a11y.spec.ts#L99) — “expanding a hop row reveals the token diff of the two readings” |
...
| happens in | [`apps/web/src/assets.ts:89`](../apps/web/src/assets.ts#L89) — `loadPage` |
...
| | |
|---|---|
| happens in | [`apps/web/src/components/PageSlider.tsx:78`](../apps/web/src/components/PageSlider.tsx#L78) — `PageSlider` |
...
| happens in | [`apps/web/src/components/PageSpread.tsx:141`](../apps/web/src/components/PageSpread.tsx#L141) — `PageSpread` |
| happens in | [`apps/web/src/useMediaQuery.ts:43`](../apps/web/src/useMediaQuery.ts#L43) — `DESKTOP_QUERY` |
| proven by | [`apps/web/e2e/desktop.spec.ts:155`](../apps/web/e2e/desktop.spec.ts#L155) — “puts the lower page number on the right” |
| proven by | [`apps/web/e2e/desktop.spec.ts:85`](../apps/web/e2e/desktop.spec.ts#L85) — “appears above the breakpoint and does not exist below it” |
...
| proven by | [`apps/web/e2e/desktop.spec.ts:344`](../apps/web/e2e/desktop.spec.ts#L344) — “still turns pages with the arrow keys the header advertises” |

> **Not proven:** Only the desktop project ever renders a spread, and it runs one file. The rest of the suite — every aria snapshot, every golden image, the axe pass — is recorded on a phone, so a regression that only shows at desktop width has exactly these four assertions standing between it and a release. Nothing checks how a spread behaves under pan and zoom, where two mounted pages share a frame budget that was measured with one.
...
|---|---|
| happens in | [`apps/web/src/components/PageSpread.tsx:141`](../apps/web/src/components/PageSpread.tsx#L141) — `PageSpread` |
| happens in | [`apps/web/src/i18n.tsx:448`](../apps/web/src/i18n.tsx#L448) — `facingAbsent` |
| proven by | [`apps/web/e2e/desktop.spec.ts:261`](../apps/web/e2e/desktop.spec.ts#L261) — “announces the missing facing page instead of showing blank paper” |
...
| | |
|---|---|
| happens in | [`apps/web/src/components/DesktopChrome.tsx:24`](../apps/web/src/components/DesktopChrome.tsx#L24) — `DesktopChrome` |
| proven by | [`apps/web/e2e/desktop.spec.ts:318`](../apps/web/e2e/desktop.spec.ts#L318) — “gives the header the controls the phone had to hide” |
| proven by | [`apps/web/src/components/DesktopChrome.test.tsx:115`](../apps/web/src/components/DesktopChrome.test.tsx#L115) — “only hints keys that actually do the thing the hint claims” |
...
| happens in | [`apps/web/src/packs.ts:159`](../apps/web/src/packs.ts#L159) — `pinPack` |
| happens in | [`apps/web/src/packs.ts:316`](../apps/web/src/packs.ts#L316) — `checkPack` |
| happens in | [`apps/web/src/components/PackShelf.tsx:79`](../apps/web/src/components/PackShelf.tsx#L79) — `PackShelf` |
| proven by | [`apps/web/e2e/offline.spec.ts:405`](../apps/web/e2e/offline.spec.ts#L405) — “opens a page the reader has never visited, with the network gone” |
| proven by | [`apps/web/e2e/offline.spec.ts:458`](../apps/web/e2e/offline.spec.ts#L458) — “a sweep is said out loud, and the juz can be put back” |
| proven by | [`apps/web/e2e/offline.spec.ts:506`](../apps/web/e2e/offline.spec.ts#L506) — “a pack the sweep only half took is not left looking kept” |
...
| happens in | [`apps/web/src/components/RevisionMap.tsx:162`](../apps/web/src/components/RevisionMap.tsx#L162) — `holdings` |
...
| proven by | [`apps/web/src/components/RevisionMap.test.tsx:206`](../apps/web/src/components/RevisionMap.test.tsx#L206) — “opens the division when its cell is pressed, and gets out of the way” |
| proven by | [`apps/web/src/components/RevisionMap.test.tsx:234`](../apps/web/src/components/RevisionMap.test.tsx#L234) — “offers no control on a cell with no paper behind it” |
...
| | |
|---|---|
| happens in | [`packages/core/src/highlighter.ts:225`](../packages/core/src/highlighter.ts#L225) — `enhancePolygons` |
...
| proven by | [`apps/web/e2e/share-a11y.spec.ts:119`](../apps/web/e2e/share-a11y.spec.ts#L119) — “keyboard-only: focus an ayah, select with Enter, open rail, hop, land” |
| proven by | [`apps/web/e2e/share-a11y.spec.ts:243`](../apps/web/e2e/share-a11y.spec.ts#L243) — “the mushaf page is a labelled group of ayah buttons” |
| proven by | [`apps/web/e2e/share-a11y.spec.ts:231`](../apps/web/e2e/share-a11y.spec.ts#L231) — “the chrome announces every control by word, never by glyph” |
...
| happens in | [`apps/web/src/i18n.tsx:828`](../apps/web/src/i18n.tsx#L828) — `LangProvider` |
...
| | |
|---|---|
| happens in | [`apps/web/src/messages/ar.json:138`](../apps/web/src/messages/ar.json#L138) — `distancePages` |
| happens in | [`apps/web/src/messages/plural.ts:39`](../apps/web/src/messages/plural.ts#L39) — `export function plural` |
| happens in | [`apps/web/src/i18n.tsx:477`](../apps/web/src/i18n.tsx#L477) — `buildStrings` |
...
17 of 40 use cases carry a `gap` — a promise whose proof is thinner than the promise. They are collected here because a hole listed once beside its own use case is a hole nobody counts.

- **The ayah is not the thing I keep confusing — a phrase inside it is. Let me point at just those words.** — The words can be picked and nothing is done with them yet: the hop still searches at ayah granularity, because print words and the QAC segmentation the edges are built from do not line up one-to-one (docs/PLAN.md 14). And the whole descent is finger-only — there is no keyboard or screen-reader path down to a word, so a reader who cannot see the page cannot reach this at all. Tracked, not assumed covered: docs/issues.json word-selection-has-no-keyboard-or-screen-reader-path.
