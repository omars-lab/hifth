...
| happens in | [`apps/web/src/components/PageStage.tsx:251`](../apps/web/src/components/PageStage.tsx#L251) — `PageStage` |
...
| happens in | [`apps/web/src/components/PageStage.tsx:1105`](../apps/web/src/components/PageStage.tsx#L1105) — `async navigateTo` |
...
| happens in | [`apps/web/src/assets.ts:33`](../apps/web/src/assets.ts#L33) — `loadPage` |
...
| proven by | [`apps/web/e2e/page-turn.spec.ts:379`](../apps/web/e2e/page-turn.spec.ts#L379) — “a sideways flick turns the page, and the band tracks the finger” |
...
| proven by | [`apps/web/src/components/PageSlider.test.tsx:71`](../apps/web/src/components/PageSlider.test.tsx#L71) — “commits on release, not on every value the thumb passes over” |
...
| happens in | [`apps/web/src/components/PageSpread.tsx:124`](../apps/web/src/components/PageSpread.tsx#L124) — `PageSpread` |
...
|---|---|
| happens in | [`apps/web/src/components/PageSpread.tsx:124`](../apps/web/src/components/PageSpread.tsx#L124) — `PageSpread` |
| happens in | [`apps/web/src/i18n.tsx:407`](../apps/web/src/i18n.tsx#L407) — `facingAbsent` |
...
| proven by | [`apps/web/src/components/RevisionMap.test.tsx:101`](../apps/web/src/components/RevisionMap.test.tsx#L101) — “draws absent as a different kind of thing from cold, not a paler one” |
| proven by | [`apps/web/e2e/revision.spec.ts:42`](../apps/web/e2e/revision.spec.ts#L42) — “a tap on an ayah is still on the map after a reload” |
| proven by | [`apps/web/e2e/revision.spec.ts:101`](../apps/web/e2e/revision.spec.ts#L101) — “a wiped record reads as a young record, not as an empty one” |

> **Not proven:** Two holes, and the first is not closable in this repo. iOS deletes script-writable storage after seven days of no interaction, which is precisely the history this promise is about. The e2e can take the record with one CDP call and prove the map still says how old it is, but it cannot wait eight days on a real phone — that is docs/validation/ledger.json → offline-survival-8-day. Second, the marquee path is proven at the store, not through the gesture: no unit test drives a real drag, so «one look, not twelve» is asserted on `recordLook` rather than on a release over twelve ayahs. Loop 4b closed the third hole this entry used to carry: the map drew two held hizb and fifty-eight absent ones, so it was a picture of an inventory rather than of a mus'haf. It is sixty of sixty now, and the absent treatment is held by a trimmed manifest instead of by the build.
...
| happens in | [`apps/web/src/i18n.tsx:761`](../apps/web/src/i18n.tsx#L761) — `LangProvider` |
...
| happens in | [`apps/web/src/i18n.tsx:436`](../apps/web/src/i18n.tsx#L436) — `buildStrings` |
...
15 of 38 use cases carry a `gap` — a promise whose proof is thinner than the promise. They are collected here because a hole listed once beside its own use case is a hole nobody counts.
