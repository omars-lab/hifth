...
| 19 | **What turns the page with a pointer** | A horizontal drag, **only at fit-zoom** | **Nothing** — the wheel, the arrows and the page bar | At z=1 the pan gesture is already a measured no-op (`holdAxis` forces the centre for an axis that fits), so the swipe slot on a phone is empty and costs no existing gesture; above z=1 a drag pans and the turn stays on the visible buttons (WCAG 2.5.1). On desktop a pointer drag already means *marquee*, and no mobile constraint put a pointer turn out of reach — the affordance a desktop actually has is the wheel, which currently does nothing at all. `page-turning.md` §3.2 ④, §4 |
