...
| 10 | **The missing facing page** | n/a | Rendered **absent** — recessed, dashed edge, its number in the printed position, and the inventory line the page bar carries («المتوفّر ٣ من ٦٠٤ صفحة» when this was written; ٦٠٤ من ٦٠٤ since Loop 4b, which is why no live caller reaches this branch any more) | Only pages 7, 9 and 19 are vendored and they are not adjacent, so there is **no facing pair in this build**. A blank sheet impersonating paper is the exact failure the page bar was built to avoid. Design doc §4 |
...
| 14 | **What an arrow key turns** | One page, through the vendored inventory | **The same** — one page, not one leaf | `stepPage` walks the inventory, and ±2 over an inventory of three is a no-op or an overshoot. The keymap is core's and tested there; a component does not get to reinterpret it. **Reopened at Loop 4b and settled the same way on different ground** — the arithmetic above died with a complete corpus, and what holds ±1 up now is that `stepPage` must name any landing that is not the page next door (so ±2 would announce a correction on every turn), that ±2 reaches only one parity and so puts half the print out of the arrows' reach, that the wheel and the dragged fold end in the same `stepPage` and would stop meaning the same thing, and that `KeyContext` would have to learn how wide the window is. Design doc §8 ① |
