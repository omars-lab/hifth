...
| **240** | QPC V4 Tajweed Font (page by page) | zip of 604 files `p1.ttf` … `p604.ttf` (69 MB; 160 MB unpacked); page N maps the private run U+FC41… to N's words — **one file per page**, so no other file (and no general Arabic face) draws them | **Yes** — every glyph is a whole printed word. Held in the ETL's gitignored `.cache/qul-fonts/qpc-v4-tajweed/`, served to the dev diff view only, never the tree or bundle. | **CLEARED 2026-09-08** — owner read the KFGQPC font terms (ledger `qul-held-copy-licensed-and-offbundle`). Fetched 2026-09-08 via the signed-in session; sha256 `b29c0282…8f6562`. The plain black cut is not on QUL; the Quran Foundation CDN serves it (`static.qurancdn.com/fonts/quran/hafs/v4/ttf/pN.ttf`). |
| **462** | KFGQPC Nastaleeq | one `KFGQPCNastaleeq-Regular.ttf` (249 KB) — a general Arabic text face, **not** the per-page word font; it draws the V4 word codepoints as unrelated ligatures | **Yes** — a Qur'an text face. Cached 2026-09-08 by mistake for the diff view; kept in the same gitignored cache, unused. | **CLEARED 2026-09-08** (same ledger check). |
